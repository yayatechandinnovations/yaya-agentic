...
| M2.8 | [M2.8-tenant-registry.md](M2.8-tenant-registry.md) | ~1 | 🟩 | First-class tenants, path-only HTTP tools, cross-tenant profile clone |
...
M0 ──▶ M1 ──▶ M2 ──▶ M2.5 ──▶ M3 ──▶ M4 ──▶ M5
                │       │       ▲
                │       │       │
                │       └──▶ M2.8 (tenant registry, path-only HTTP tools,
                │                  cross-tenant profile clone — slots before
                │                  M3 because eval reuses tenant scoping)
                └──┬────┘ (M2 and M2.5 overlap weeks 8–10; RAG depends on M2 prompt
                          assembly hooks but engine work continues in parallel)
```

M2.8 is a sideways extension of the trust model: M2.5's `AccessRequirement` on knowledge sources and M2's `Authorizer` chain on tools both already exist; M2.8 promotes the **tenant** itself to a first-class trust root that owns the host base URL, inbound origins, default auth, and default recorder. M3 (eval) starts as soon as M2 prompt assembly is stable enough to record traces against.
